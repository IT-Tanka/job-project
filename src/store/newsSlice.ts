...
>("news/fetchNews", async function (num, { rejectWithValue, getState }) {
  const news = getState().news.list;
  const response = await fetch(
    `https://jsonplaceholder.typicode.com/posts?_limit=5&_page=${num}`
  );
...
export const deleteNews = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("news/deleteNews", async function (id, { rejectWithValue }) {
  const response = await fetch(
